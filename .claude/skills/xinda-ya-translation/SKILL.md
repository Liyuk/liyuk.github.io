...
For the English-side AI-pattern pass, this project also installs `.claude/skills/humanizer/SKILL.md`. Apply it only after the translation satisfies 信 and 达: it is a final English prose review for generic framing, inflated claims, repetitive structure, chatbot residue, and rhythm. It supplements the translation standard and never overrides source fidelity, heading structure, protected facts, or the author's calibrated voice.

## Producing a new `en.md`

1. Confirm the `zh.md` is actually final (see the relevant `agent/category-guides/*.md` for that collection's drafting order) and select the matching source-state branch in `agent/translation-spec.md`.
...
4. Run `.claude/skills/humanizer/SKILL.md` as an English prose pass. Keep the Chinese argument, claims, certainty, citations, proper nouns, and required Markdown structure fixed; use Humanizer to remove English AI tells without adding personal detail or making the translation artificially casual.
5. For an unpublished entry, set `translationStatus: draft`. A first-pass translation remains draft until it completes the full 信达雅 review and the site owner accepts `reviewed`; a published Chinese source follows the candidate path in `agent/translation-spec.md` instead.
...
4. **Humanizer pass** — after 信 and 达 are secure, apply `.claude/skills/humanizer/SKILL.md` to flag inflated significance, vague attribution, formulaic contrasts, chatbot residue, repeated framing, unnecessary headings, and uniform rhythm. Keep deliberate first-person texture and any phrasing needed for faithful meaning; Humanizer is an audit/editing aid, not an excuse to erase the author's voice.
5. **Term glossing** — where the Chinese relies on institutional shorthand or a proper noun (job-level labels, company-size shorthand, platform names) that an English-only reader has no context for, is there a short descriptive gloss per `agent/translation-spec.md`'s "Term glossing" section? Flag both directions: a load-bearing term left opaque, and a gloss that oversteps into adding an opinion or claim the Chinese didn't make.
