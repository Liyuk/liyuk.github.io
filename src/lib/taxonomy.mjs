...
export const columns = {
  ...
  'technical-systems': {
    ...
    description: { 'zh-CN': '从业务约束出发做技术规划，让架构、规范与代码在演进中保持可判断。', en: 'Planning from business constraints; keeping architecture, standards, and code sound as systems evolve.' },
  },
  'team-building': {
...
function localized(copy, locale) {
  return copy[locale === 'en' ? 1 : 0];
}

export function getTag(slug, locale = 'zh-CN') {
  const tag = tags[slug];
  return { slug, label: tag ? localized(tag, locale) : slug };
}

export function getColumn(slug, locale = 'zh-CN') {
  const column = columns[slug];
  if (!column) return null;
  return {
    slug,
    label: column.label[locale] ?? column.label['zh-CN'],
    description: column.description[locale] ?? column.description['zh-CN'],
  };
}

export function getColumnEntries(entries, columnSlug) {
  return entries
    .filter((entry) => entry.data.column?.slug === columnSlug)
    .sort((a, b) => a.data.column.order - b.data.column.order || a.id.localeCompare(b.id));
}

export function getRelatedEntries(entries, currentEntry, limit = 3, locale = 'zh-CN') {
  const currentTags = new Set(currentEntry.data.tags ?? []);
  const en = locale === 'en';

  // 排除同专栏相邻篇（已在专栏导航展示，避免推荐重复）
  const columnNeighbors = new Set();
  ...
  const scored = entries
    ...
    .map((entry) => {
      ...
      // 1. 同专栏：强相关但不独占
      const sameColumn = currentEntry.data.column?.slug && entry.data.column?.slug === currentEntry.data.column.slug;
      if (sameColumn) {
      ...
      if (shared.length > 0) reasons.push(en ? `shared ${shared.length} tag${shared.length > 1 ? 's' : ''}` : `共享标签 ${shared.length} 个`);
  ...
  return result.map(({ entry, reasons }) => ({ entry, reasons }));
}

export function getIndexableTags(entries, minimumEntries = 3) {
