...
这套原则是从一次具体的困扰里推出来的（下面举例来说明，为构造场景，不是逐字的真实记录，但反映了维护长期写作博客时会遇到的典型问题）：一个写了几年的个人博客，标签越加越多，某天想给一组"从入门到进阶"的系列文章加个顺序，结果发现能用的只有标签——于是往标签里塞了 `series-xxx-01`、`series-xxx-02` 这种编号，标签页瞬间被污染，既回答不了"这篇属于什么主题"，也回答不了"该按什么顺序读"。同样的问题也出现在项目和研究上：项目有没有在维护、研究是不是已经过时，这些状态信息也被硬塞进标题或摘要里，因为内容模型里没有为它们留位置。补一次是能修，但每次新增一种阅读方式就要重新发明一套字段命名，这才是需要先定下内容模型、再决定页面怎么长的原因。
...
这一段同样是构造示例，用来说明取舍逻辑，而不是某次真实选型会议的记录。

在动手写 Fourfold 之前，摆在面前的其实是三条更省事的路：

- **直接用 Ghost / WordPress 之类的 CMS。** 优点是编辑体验和评论、订阅这些能力都是现成的；但代价是要维护一个数据库和后台，内容和展示逻辑绑得很死，想把"专栏篇序"这种非标准字段塞进去，往往要么改插件，要么在正文里手写 HTML，长期看维护成本比自己搭一层内容模型更高。
- **用 Hugo/Jekyll 这类成熟静态博客框架，直接套现成主题。** 静态优先这一点是对的，省去了自己写构建流程的工作；但现成主题的内容模型通常只有"文章 + 分类 + 标签"，没有"专栏"和"项目状态"这类语义，硬塞进去意味着要么滥用 front matter，要么维护一堆和主题模板耦合的自定义字段，一旦换主题就得重来。
- **只用 Notion/飞书文档对外发布。** 几乎零构建成本，但没有独立域名下的 RSS、sitemap 和长期归档，内容的"出版物"感和可控性都不够，也不方便做站内的标签、专栏这类横向组织。

最后选择自己搭一套四层模型（内容 schema、查询层、页面层、发布层），本质上是在"要一个能表达专栏/项目状态这类语义的内容模型"和"不想自己维护后台"之间找中间地带：静态构建保留了 Hugo 类方案的部署简单性，但内容 schema 和查询层是按这个作者实际需要的阅读方式（专栏、标签、项目状态）专门设计的，而不是套用某个通用主题的字段。代价是初期要自己写 schema 校验和查询逻辑，比装一个现成主题慢。
...
发布流程的状态边界比页面本身更重要。内容依次经过草稿（Draft）、内容校验（Validating）、类型检查通过（Checked）、构建（Building）、可发布产物（Ready）、发布（Published）、归档（Archived）几个状态；任何一步失败都回到可以修复的上一个状态，而不是继续往下生成一个半成功的页面。`Draft`、`Checked` 和 `Published` 不是同一个概念：前者是作者工作状态，中间状态是工程检查结果，后者才是外部读者能够访问的发布结果。
...
目前 Fourfold 完成了内容 schema、查询层、六种内容类型的页面、静态搜索、RSS/sitemap 和一致性校验；还没做完的是评论、邮件订阅、跨设备收藏这几项——它们需要外部服务，暂时留了接口位置但没有接入。如果只是想快速搭一个个人写作站点、不需要专栏和项目状态这类结构，直接用现成的静态博客主题可能比接入这套四层模型更快；这套模型主要是为内容类型和阅读方式会持续变多的长期写作场景准备的。
