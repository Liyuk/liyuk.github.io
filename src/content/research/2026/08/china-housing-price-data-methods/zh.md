...
中国城市房价数据在公开层面通常可获取，但其"可测量"边界远比表面复杂：官方 70 城指数是价格指数而非成交单价，区级官方数据往往只有成交量而没有成交价格，第三方挂牌样本则属于另一套口径。本文提出一种**来源—口径感知（source- and methodology-aware）的数据构建方法**，把"数字能告诉我们什么 / 不能告诉我们什么"显式编码进数据本身，而不是留给模型去猜测。

本文以中国 70 城官方指数面板为城市层基准，全国宏观面板（房地产开发投资、销售、库存及其同比）与央行 LPR 序列为宏观经济背景，聚焦北京市作为区级案例：通过来源分级标签（官方 / 行业指数 / 公开第三方 / 派生）、口径切换标记、时间与空间泄漏控制，构建了一套可复现的数据资产。实证部分给出四组发现：(1) 北京二手住宅官方同比指数在补全后形成 2019-01 至 2026-06 连续 90 个月的完整序列，峰值与谷值分别为 2021-07 的 110.7 与 2024-09 的 89.7；(2) 全国宏观面板与 LPR 序列补全后覆盖 2019-08 至 2026-07，支撑对房地产开发投资持续下行的量化描述（2023-11 累计同比 −9.4% → 2026-06 −18.0%）；(3) 官方二手环比与区级挂牌均价环比在抽样月份的变动方向一致性仅为 1/5，说明"价格"在不同口径间并不必然同向移动；(4) 官方区级网签（2025-10，17 区）提供了成交量层面的可靠横截面，但与区级价格之间没有连续面板可衔接。方法贡献在于一套可复用的数据边界声明协议，而非一个预测模型。
