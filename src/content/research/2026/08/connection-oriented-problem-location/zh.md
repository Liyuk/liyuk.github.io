...
还需要回答一个更尖锐的问题：市面上已有若干"从现状到根因到方案"的完整流程方法，五镜法比它们多了什么、少了什么。这里只与结构最接近的 Kepner-Tregoe（KT）详细对齐，其余框架简单提及边界，不逐一列表对比（KT、TRIZ、Senge 的出处见参考文献，均已核验）。

**与 KT 的对齐。** KT 的流程是情境评估→问题分析→决策分析→潜在问题分析；五镜法给它的"问题分析"一步补上一层显式透镜（尤其是 KT 不强调的立场/利益两镜），但没有 KT 的"潜在问题分析（PPA）"这道收尾工序——两者可以互补而非互相替代。

**与其余框架的边界，一句话带过：** 丰田 A3 把根因收敛成一页纸便于现场执行，五镜法则把根因拆成五类联系、不如 A3 收敛；Six Sigma DMAIC 与 TRIZ 矛盾矩阵都依赖数据或技术领域知识（统计工具、发明原理库），五镜法更适合数据不足、偏人与判断的问题，但不能替代它们的专业工具箱；系统思考（Senge，1990）与设计思维分别在"建模反馈回路"和"原型/用户测试"上比五镜法更重，五镜法胜在更轻、不要求建模或原型；第一性原理拆解到不可再拆的基本事实，五镜法多一层"联系"但不如它对物理/工程问题直接。
...
本文是这套方法的"立论"版本；只想照做的读者，可看它的实践版[《思维训练（四）：结构化思维实践》](/writing/2026/08/structured-thinking-practice/)。
