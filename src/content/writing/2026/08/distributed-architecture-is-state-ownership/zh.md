---
title: 从一次请求看多节点执行系统的架构演进
description: 从一套多节点执行系统的故障结构出发，复盘请求、状态、存储、性能和故障如何逐渐跨越边界，并讨论下一版架构应该怎样重新组织。
...
## 一、执行系统是怎样从一条调用链路长出来的

### 从单节点开始

单节点执行系统的很多问题可以被进程和数据库暂时隐藏。协调层知道后端资源在哪里，后端资源知道自己的状态，日志也通常写在同一处。即使没有正式的服务发现、容量模型和故障转移，它仍然可能满足当前规模的需求。

但这并不意味着问题不存在。它们只是被“只有一个地方”这个条件遮住了。

### 增加节点以后

当执行系统拥有多个执行节点时，系统马上需要回答：请求应该送到哪个节点，节点是否真的可用，节点支持哪些能力，当前还剩多少容量，以及失败后切换是否会重复执行。

增加多个区域以后，全局入口、区域服务和本地后端资源又会分别拥有局部信息。它们可能在不同时间观察到不同状态，也可能因为网络分区而暂时无法互相确认。这时执行系统已经出现了分布式语义，即使它只有少量机器。
