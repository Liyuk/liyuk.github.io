...
description: 从一个小型 API 服务的观测问题出发，借鉴成熟可观测平台的经验，整理 Request ID、分布式 Trace、服务发现、健康监测、日志查询、稳定性和基础设施治理的最小实现。
...
最近我在整理一些小工具的工程经验，其中有一类小型 API 服务已经具备基本功能，但观测和故障治理仍然比较粗糙。它对外提供统一的 API 入口，负责请求转发和外部能力调度：请求先经过边缘接入层，再进入若干后端服务；服务会访问数据库、对象存储，也会调用其他外部依赖。

我想做的事情，更准确地说是给这类分布式架构补上治理：补齐服务边界、请求关联、运行状态、故障处理和数据校准这些容易缺失的部分。第一步不是重画架构图，而是处理一个“请求为什么这么慢”的合成案例。直觉上，边缘接入和后端之间的网络路径很容易成为怀疑对象；但给各个环节补上打点、画出耗时火焰图以后，真正拖慢请求的是服务内部的一段计算任务。于是我针对这段任务做了一次优化，也用数据校准了原先的猜测。
...
这次整理的重点不是能不能做出一个 Datadog，而是一个小项目至少需要哪些能力，才能让“我猜是网络路径”变成“证据显示是内部计算”，也才能让失败后的重试、校准和恢复不再依赖人工猜测。

## 先把问题说准确

真正的问题不是“怎么给一个请求生成 Trace”，而是一个已经能够提供服务的系统，如果没有系统性建设可观测能力，出了问题就只能靠不完整的线索猜测。

它的问题看起来很分散：某个请求失败了，但入口只知道返回了错误；某个后端服务说自己处理过请求，但没有办法证明它和入口看到的是同一个请求；某个计算节点上的服务还在运行，可是业务已经不能正常工作；数据库可以建立连接，却可能已经出现慢查询或备份失败；对象存储也可以调用，但对象写入、读取和生命周期是否正常，没有统一的验证结果。
...
这张图可以帮助理解系统结构，但不能证明一次请求确实走过这些节点。Service A 可能没有记录请求，Service B 可能只写了一行错误，外部依赖可能有自己的 request ID，接入层只知道连接阶段发生了什么，最后没有任何一个 ID 能把它们串起来。
...
而这类 API 服务的难点也不只是把请求处理完。它还要处理外部能力的资源登记、请求调度、并发限制、失败切换和用量事件。业务代码可以把一次调用成功地完成，但从系统治理的角度看，还要确认这次调用经过了哪个实例、消耗了哪类资源、统计事件有没有落下，以及失败以后能不能安全重试。
...
系统里原本已经有很多局部 ID：入口的请求信息、某个服务自己的 request ID、外部平台的关联信息、数据库请求日志里的连接信息。它们都可能有用，但不能强行合成一个 ID。
...
这里有一个很容易被架构图掩盖的限制：链路上有一个 `trace_id`，不代表每一段时间都已经被准确测量。系统通常能比较稳定地拿到入口总耗时、下游服务的处理耗时和请求结束状态；接入层到后端之间的单次请求耗时，取决于是否在边界之间额外记录；客户端看到的 DNS、连接建立、依赖响应和最后一个字节，则不一定能从服务端补回来。

所以耗时火焰图应该区分“已采集的时间”和“根据父子 span 推断的时间”。例如入口的 wall time 和后端记录的 latency 可以直接比较，但不能把两者的差值直接宣布成某个网络层耗时：其中可能还包括平台调度、网络传输和未被埋点的处理。这个限制反而很重要——观测系统应该告诉人哪里有证据，也要告诉人哪里只是估算。

### 一次慢请求：原本怀疑网络路径，最后优化了内部计算

这次排查的过程很简单，但它改变了我对这套系统的理解。

在这个合成案例里，请求变慢时，先按网络路径猜测：边缘入口、连接层、计算节点，最后经过多个后端服务，网络层可能是最值得怀疑的环节。没有打点时，这个猜测听起来并不荒谬，但它仍然只是猜测。入口只有一个总耗时，服务日志里也只有零散的开始和结束记录，没人能说清楚时间究竟花在了哪一段。

我先没有调整网络层，而是给入口、服务调用、内部计算任务和请求结束补上时间点。把这些事件按照 `trace_id` 组织成耗时火焰图以后，结果很清楚：入口的整体耗时和后端处理时间基本能够对上，网络层带来的额外开销很小；真正占大头的是某个服务内部的计算任务，而且它位于请求的关键路径上。

这时问题就从“网络是不是慢”变成了“这段计算为什么慢”。我把这段任务继续拆成更小的步骤，确认最耗时的部分，再针对它做了一次优化。优化之后重新观察同类请求，重点比较的也不再是某一个总耗时，而是内部任务、服务处理和端到端耗时是否一起下降。

这个案例没有证明网络层永远不会成为瓶颈，它证明的是：在没有证据以前，架构图里的某个边界很容易抢走注意力。耗时火焰图的价值，不是替人自动找到答案，而是把猜测变成可验证的假设；即使最后结论是“网络层没有明显开销”，这也是一次有价值的排除和校准。

我还把不同可靠性的数据分开处理。请求结束时写入一条可查询的 Trace 摘要，更完整的诊断内容归档到对象存储；运行时日志则由采集器单独收集。它们共享 `trace_id`，但不能假设“业务请求成功”就意味着“所有观测数据都已经成功落盘”。严格业务事件、尽力而为的统计事件和诊断数据，需要有不同的失败策略。
...
只看请求链路还是不够。有些故障发生在请求进来以前：计算节点磁盘快满，容器或进程不断重启，数据库备份连续失败，对象存储的写入检查失败，或者接入层看起来连接正常，但服务的核心逻辑已经不能正常工作。
...
因此我没有把健康检查设计成一个布尔值。我记录检查类型、检查时间、耗时、结果、错误信息、版本和设备身份。对于对象存储，除了 API 可用性，还检查测试对象的写入、读取和生命周期；对于数据库，除了连接，还看慢查询和备份；对于业务服务，则加入真正的核心业务逻辑检查。
...
这时直接重试可能会造成重复写入，所以需要幂等键、状态机、重试记录、补偿任务和修复审计。校准任务会定期对比数据库记录、对象存储对象、消息消费状态、Trace 摘要和业务最终状态，找出缺失、重复、超时或互相矛盾的数据。

一个典型情况是：主请求已经完成，用户也拿到了成功响应，但请求结束后的统计和诊断事件还在异步处理。事件先进入进程内队列，随后批量写入数据库；更完整的诊断摘要则写入对象存储。只要服务在批量写入前重启，或者数据库、对象存储在那个时间窗口内出现短暂失败，主流程并不会自动回滚，最后就会出现“请求成功了，但统计缺一条”“数据库有摘要，但归档对象缺失”这样的状态。

这件事让我把请求主流程和旁路数据分成了不同等级。严格业务结果不能只依赖内存队列；尽力而为的统计可以允许延迟，但必须能发现丢失；诊断归档可以稍后补写，但要通过 `trace_id` 找回原始请求。校准任务会先用 Trace 摘要找出应该存在的事件，再对比数据库记录、消息确认状态和对象存储对象，生成一条补偿任务，而不是直接把某个表的状态改成“已完成”。

补偿也不能简单地把整条请求再执行一次。修复任务会携带原始业务幂等键、事件版本和已经完成的步骤：如果数据库记录已经存在，就跳过重复写入；如果归档对象缺失，就只补写归档；如果统计事件没有确认，就重新投递并记录新的 attempt。这样一次失败最终会留下完整的过程证据，也能在修复后再次校准。
...
计算节点、数据库、对象存储和观测数据需要分别考虑恢复路径。容灾设计必须回答：服务在哪类设备上重新启动，数据库能恢复到哪个时间点，归档对象是否还能找回，服务注册信息是否需要重建，以及观测系统本身不可用时业务系统能不能继续运行。
...
我没有打算一开始就把工业系统的所有能力搬过来。对这个小型服务来说，输入一个 `trace_id`，能看到服务节点、耗时火焰图、失败阶段、实例信息和关联日志，再从服务页看到计算节点、数据库和对象存储的健康状态，这套排查闭环就已经有用了。
...
对象存储更适合保存受控的诊断摘要和归档数据。对象需要有 schema 版本、大小上限、稳定的 key、字段白名单和生命周期规则。默认不保存敏感字段、授权头、请求正文和响应正文；写入成功还不够，还要验证对象能否被读回，并确认过期策略真的生效。
...
把数据放进对象存储以后，我才开始认真算这套系统的成本。直觉上大家会先担心“存几年会不会很贵”，但对这个项目来说，长期存储反而比较容易控制：可以设置生命周期、限制保留时间、压缩诊断摘要，也可以把不常查询的内容归档。
...
6. 监控计算节点、容器或进程、端口、readiness 和业务逻辑稳定性；
7. 监控数据库连接、慢查询和备份；
8. 检查对象存储的写入、读取和生命周期；
9. 监控域名、证书、接入层和观测系统自身的状态。

第三阶段是可靠性和数据治理：

10. 为消息消费记录确认、重试、幂等、死信和积压；
11. 对超时、降级、熔断和恢复建立独立事件；
12. 用校准任务检查数据库、对象存储、消息状态和业务状态的一致性；
13. 为计算节点、数据库、对象存储、服务注册和观测数据定义备份、恢复和容灾路径。
...
我从一个小项目开始，做的只是一套简配版。它没有工业系统的规模、容灾等级和自动化程度，但在按阶段把这些能力拼起来以后，我更具体地理解了大型日志系统、分布式追踪、服务组件、消息系统和基础设施治理为什么要被拆成不同的系统，又为什么最后必须在同一个排查入口里串起来。
